...
useful API for solving this problem (there are likely more!): bl 
(Buffer List) and concat-stream; take your pick!
...
And it will download and install the latest version of the package into 
a subdirectory named node_modules. Any package in this subdirectory 
under your main program file can be loaded with the require syntax 
without being prefixed by './':
...
will collect the data for you. Once the stream has ended, a callback 
will be fired with the data:
...
pipeFinishedCallback = function (err, data) {
	if (!err) {
		// 1. print character count
		var str = data.toString();
		console.log(str.length);
		// 2. print all the data to screen
		console.log(str);
	}
}
...
httpGetCallback = function(response) {
	var bl = require('bl'); //buffer list, 3rd party installed via 'npm'
	...
	response.pipe(
		bl(pipeFinishedCallback)
	);
}


//main code -- the request!
http.get(
	theURL,
	httpGetCallback
);
