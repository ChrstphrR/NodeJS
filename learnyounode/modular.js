...
The filename extension argument must be the same as what was passed to 
your program. Don't turn it into a RegExp or prefix with "." or do 
anything except pass it to your module where you can do what you need 
...
fs.readdir(), the callback must be called with the error, and only 
...
//include my module -- which only has ONE function in it.
...
mymodule(
	pathname,
	extension,
	function (err, data) { //inline function callback
		if (err) {
			console.error(err); //print informative message to console.
		} else { 
			//data contains an array of 0+ files matching:
			for (index in data) {
